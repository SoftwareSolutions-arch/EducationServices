...
exports.CreateCollege = function (req, res) {
    ...
    if (req.files.collegeImg) {
        var sampleFile = req.files.collegeImg;
        var Images = [];
        var dir = (__dirname + '/../../uploads/')
        if (!fs.existsSync(dir)) {
            fs.mkdirSync(dir, 755);
        ...
        sampleFile.mv(dir + UId + "_" + sampleFile.name, function (err) {
            if (err) {
                ...
                UserData.collegepicture = Images[0].name
                return AuthServices.CreateColleges(UserData).then(function (CollegeItem) {
...
exports.ShortListCollege = function (req, res) {
    var params = req.body;
    return AuthServices.CreateShortlistColleges(params).then(function (CollegeItem) {
        successHelper.returnSuccess(true, res, 200, "College shortlisted Successfully.", CollegeItem);
    }).catch(function (error) {
        console.log("error" + error)
        res.json({ "ErrorCode": error, Status: "Status", "user": "result", "ErrorMessage": "ErrorMessage" });
    })
...
exports.GetAllShortListCollege = function (req, res) {
    var params = req.body;
    return AuthServices.AllShortlistColleges(params).then(function (CollegeItem) {
        successHelper.returnSuccess(true, res, 200, "shortlisted list.", CollegeItem);
    }).catch(function (error) {
        console.log("error" + error)
        res.json({ "ErrorCode": error, Status: "Status", "user": "result", "ErrorMessage": "ErrorMessage" });
    })
...
exports.UpdateCollege = function (req, res) {
    var collegeparams = JSON.parse(req.body.jsonInput);
    if (collegeparams.collegeprofile == true) {
        var sampleFile = req.files.collegeImg;
        var Images = [];
        var dir = (__dirname + '/../../uploads/')
        if (!fs.existsSync(dir)) {
        ...
        sampleFile.mv(dir + UId + "_" + sampleFile.name, function (err) {
            if (err) {
                ...
                var params = {
                    "title": (collegeparams.title) ? (collegeparams.title) : "",
                    "description": (collegeparams.description) ? collegeparams.description : "",
                    "highlight": (collegeparams.highlight) ? collegeparams.highlight : false,
                    "facilitie": (collegeparams.facilitie) ? collegeparams.facilitie : false,
                    "url": (collegeparams.url) ? collegeparams.url : false,
                    "country": (collegeparams.country) ? collegeparams.country : false,
                    "branches": (collegeparams.branches) ? collegeparams.branches : false,
                    "address": (collegeparams.address) ? collegeparams.address : false,
                    "infrastructure": (collegeparams.infrastructure) ? collegeparams.infrastructure : false,
                    "exam": (collegeparams.exam) ? collegeparams.exam : false,
                    "collegetype": (collegeparams.collegetype) ? collegeparams.collegetype : false,
                    "course": (collegeparams.course) ? collegeparams.course : false,
                    "fees": (collegeparams.fees) ? collegeparams.fees : false,
                    "city": (collegeparams.city) ? collegeparams.city : false,
                    "_id": (collegeparams._id) ? collegeparams._id : false,
                    "images": (Images[0].name) ? Images[0].name : false,
                }
                return AuthServices.UpdateCollege(params, collegeparams._id).then(function (Data) {
        ...
        return AuthServices.UpdateCollege(params, collegeparams.url).then(function (Data) {
            res.json({ "Status": 200, "result": Data, "Message": "College has been updated successfully." });
        }).catch(function (error) {
            res.json(error);
        });
...
exports.GetDataCollege = function (req, res) {
    var params = req.body
    return AuthServices.GetDataColleges(params).then(function (Data) {
        if (Data) {
            res.json({ "Status": 200, "result": Data, "Message": "College list." });
        } else {
            var Data = [];
...
exports.GetAllCollegeData = function (req, res) {
    return AuthServices.GetAllCollegeData().then(function (Data) {
        if (Data) {
            res.json({ "Status": 200, "result": Data, "Message": "All College list." });
        } else {
            var Data = [];
            res.json({ "Status": 200, "result": Data, "Message": "All College list." });
        }
...
exports.GetCollegeById = function (req, res) {
    ...
    return AuthServices.GetCollegeById(params).then(function (Data) {
        if (Data) {
            res.json({ "Status": 200, "result": Data, "Message": "College list." });
        } else {
            var Data = [];
...
/****************Controller for GetCollegeDataByLimit */
exports.GetCollegeDataByLimit = function (req, res) {
    var params = req.body.skipvalue
    return AuthServices.GetCollegeDataByLimit(params).then(function (Data) {
        if (Data) {
            res.json({ "Status": 200, "result": Data, "Message": "College list." });
        } else {
            var Data = [];
...
/****************Controller for get College */
exports.GetTopDataCollege = function (req, res) {
    return AuthServices.GetTopDataCollege().then(function (Data) {
        if (Data) {
            res.json({ "Status": 200, "result": Data, "Message": "Top College list." });
        } else {
            var Data = [];
            res.json({ "Status": 200, "result": Data, "Message": "Top College list." });
        }
...
exports.DeleteCollege = function (req, res) {
    ...
    return AuthServices.Deletecollege(whereparams).then(function (Item) {
        res.json({ "Status": 200, "Message": "College  Deleted Suucessfully." });
    }).catch(function (error) {
...
exports.VerifyOpt = function (req, res) {
    ...
    return AuthServices.findUserbyOtp(whereparams).then(function (Data) {
        ...
        res.status(500).json({ Status: 500, succeed: false, message: "Something went wrong.", errorsdata: error })
    });
}
/**** Lopgout User  */
...
exports.GetDashboardData = function (req, res) {
    ...
    return AuthServices.getDashboardData(req.AuthData._id, req, res).then(function (Data) {
        ...
        res.status(500).json({ code: 500, succeed: false, message: error, errorsdata: error })
    });
}
...
exports.SendMailforForgetPassword = function (req, res) {
    ...
    return AuthServices.getForgetpasswordMail(email).then(function (Data) {
        ...
        res.status(500).json({
            code: 500, succeed: false, message: "Something went wrong.",
            errorsdata: error
        })
    });
}
...
exports.CreateQuestion = function (req, res) {
    let questionpartams = req.body;
    return AuthServices.CreateQuestion(questionpartams).then(function (Result) {
        res.json({ "Status": 200, "Message": "Question created successfully.", "result": Result });
    }).catch(function (error) {
        return error;
    })
}
...
exports.UserComment = function (req, res) {
    ...
    return AuthServices.UserCommenting(answerparams).then(function (Result) {
        res.json({ "Status": 200, "Message": "Answer Submitted Successfull.", "result": Result });
    }).catch(function (error) {
        return error;
    })
}
...
exports.AllQuestion = function (req, res) {
    return AuthServices.GetAllQuestion().then(function (Result) {
        if (Result) {
            res.json({ "Status": 200, "Message": "Question list Successfull.", "result": Result });
        } else {
            res.json({ "Status": 200, "Message": "Question list Successfull.", "result": [] });
        }
    }).catch(function (error) {
        return error;
    })
...
exports.AllQuestionList = function (req, res) {
    ...
    return AuthServices.GetAllQuestionList(params).then(function (Result) {
        if (params == "all") {
            Result.all = params;
            res.json({ "Status": 200, "Message": "Question list Successfull.", "result": Result });
        } else if (params == "discussion") {
            Result.discovered = params;
            res.json({ "Status": 200, "Message": "Question list Successfull.", "result": Result });
        } else if (params == "unanswered") {
            Result.discovered = params;
